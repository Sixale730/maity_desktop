...
- `ffmpeg.exe` + `ffmpeg-LICENSE.txt` → los deja ya `frontend/scripts/stage-ffmpeg-windows.js`, que corre dentro de `tauri:build:store` y copia el binario pineado de `frontend/src-tauri/binaries/ffmpeg-x86_64-pc-windows-msvc.exe` (y su LICENSE) al staging **en cada corrida, tambien cuando el stamp ya esta al dia** — el staging esta gitignored y se recrea entre ciclos, asi que copiar solo tras una descarga fresca dejaria el MSIX sin ffmpeg (⚠️ el build release con `--no-bundle` NO copia externalBin al output; por eso el stage lo hace a mano). Es el prebuilt **LGPL** de BtbN verificado por SHA-256, no el GPLv3 de gyan.dev que la app descargaba antes. **SIN `ffprobe.exe`**: el código no lo ejecuta en ninguna parte (cero coincidencias en `src-tauri/src`) y eran 94.5 MB de peso muerto y superficie de escaneo en la Store — si sigue en el staging de una corrida vieja, **borrarlo** (el stage script ya lo hace).
- **`msvcp140.dll`, `msvcp140_1.dll`, `vcruntime140.dll`, `vcruntime140_1.dll`** → de `frontend/src-tauri/vcredist/` (los deja ahí `scripts/stage-vcredist.js`, que corre dentro de `tauri:build:store`). **OBLIGATORIOS** — sin ellos la app no arranca en un Windows limpio; es lo que rebotó la certificación 10.2.4.1 (ver Gotchas). El cierre (qué DLLs del VC++ Runtime importan el exe y `llama-helper.exe`) lo verifica `scripts/lint-exe-imports.js` al final de `tauri:build:store` contra el contenido real de `vcredist/`; si falla nombrando un DLL nuevo, primero quitar la dependencia (así se resolvió `vcomp140.dll`/OpenMP en sep-2026) y solo si es imprescindible sumarlo a `stage-vcredist.js` Y a esta lista.
...
- **⚠️ `winapp run` NO registra el staging tal cual** (descubierto 2026-07-20): construye una subcarpeta `<staging>\AppX\` con SOLO lo que el manifest declara (exe + `Assets\` + pri) y **descarta el payload suelto** (ffmpeg.exe, llama-helper.exe, templates\, y los 4 DLLs del VC++ Runtime). Síntoma: la app corre y transcribe, pero `recording_saver` truena en cada chunk con `Failed to spawn FFmpeg process: os error 2` (miles de errores) y el coach no arranca (sidecar ausente). **Fix: DESPUÉS de `winapp run`, copiar esos elementos dentro de `<staging>\AppX\`** — sigue funcionando **en caliente** con la app corriendo: `CreateProcess` busca primero en el dir del exe, el incremental saver retiene el PCM en RAM y drena el backlog al aparecer ffmpeg, y desde sep-2026 (#32) el resolver de `audio/ffmpeg.rs` es **reintentable** (`OnceLock` que sólo cachea el éxito), así que un fallo previo ya no queda memorizado para toda la sesión — antes, con el `Lazy<Option<PathBuf>>`, el rescate en caliente sólo funcionaba si ningún encode había intentado resolver todavía. ⚠️ Los DLLs del VC++ Runtime son la excepción: se cargan al **arrancar** el proceso, así que copiarlos en caliente no sirve — tienen que estar en `AppX\` antes de lanzar. Verificar en logs (`%LOCALAPPDATA%\Maity\logs\`) que los `Saved checkpoint N` fluyan cada 30s sin errores.
