...
- **Formato de 4 partes.** MSIX exige `X.Y.Z.R`; el semver de Tauri tiene 3. Sufijar siempre `.0` (0.2.53 → `Version="0.2.53.0"`). **El 4º componente (revision) DEBE ser SIEMPRE 0**: Partner Center rechaza el paquete con *"Apps are not allowed to have a Version with a revision number other than zero"* (rebote real 2026-07-27) — ese digito se lo reserva Microsoft, NO sirve para re-submissions. Una re-submission con el mismo codigo requiere bump de `Z`; el bump normal solo mueve X.Y.Z.
