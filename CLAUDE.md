...
- **Rearme de la jornada con causa (`UserStop|AutoClose|SessionEnd`)**: toda ruta de salida, logout o apagado marca `session_ending` (`begin_session_end`) ANTES de detener y NUNCA escribe supresión de día. Solo `UserStop`/`AutoClose` persisten (`scheduled_recording_runtime.json`, único escritor `update_rearm`). Tras el cierre automático el skip es `closed_for_day` hasta el siguiente inicio de ventana (el turno nocturno no pierde sus horas antes de medianoche); "Evaluar ahora" usa `reset_immediately`. Detalle: `docs/ONBOARDING_Y_GATES.md` § Supresión de la jornada persistida (0.2.62, #83 J4 / B4).
...
- **TODO logout (Ajustes, Sidebar, sidebar del chat) pasa por `AuthContext.signOut(surface)`**, nunca por el signOut de supabase-js directo (guard `contexts/authSignOut.fitness.test.ts`). Recargar el webview NO suelta al usuario en Rust (`lib/authRelease.ts::shouldReleaseRustUser`); una sesión perdida a media grabación espera `session_lost_cleanup` antes de `clear_current_user`. Detalle: `docs/NUBE_CUENTAS_SYNC.md` § Todo logout pasa por AuthContext.signOut (sep-2026, #83).
...
**¿Por qué no grabó? (#83, desde 0.2.62):** `app.start` (causa del cierre anterior), `app.exit` (motivo), `app.resumed`, `autostart.changed`, `auth.logout`/`auth.session_lost` y `jornada.settings_changed`/`jornada.idle_reason_changed` van todos por el outbox; `idle_reason` + bloque `jornada` van en `health.heartbeat` (`device.profile` lleva solo la configuración de jornada). La verdad del ciclo de vida es el marcador síncrono `lifecycle.json` (`logging/telemetry/lifecycle.rs`); la fila es best-effort. `app.close` NO es la salida del proceso (es la ventana que se va a la bandeja) y la desinstalación se infiere. Query persona × día: `docs/TELEMETRIA.md` § ¿Por qué no grabó? — persona × día hábil (#83).
