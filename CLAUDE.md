...
Analiza un piloto empresarial desde Supabase y produce **dos entregables separados**: (A) notas técnicas internas en `docs/PILOTO_<EMPRESA>_<fecha>.md` (RAM, scheduler, atribución, cuota, calidad de datos) y (B) un artifact HTML para el manager del cliente que convierte datos en **conductas** — 6 competencias con por qué y qué mejorar (cita → alternativa), tarjeta por persona con nombre (puesto y autoevaluación de `form_responses`, reto declarado vs medido), tipos de junta desde la minuta (`meta.tipo_reunion × categoria_interlocutor`), cobertura de jornada (9–18 L–V por default) con los picos explicados, qué hacen los que no graban, acciones. Regla de origen (feedback del cofundador, ago-2026): **nada de KPIs de volumen ni jerga ni problemas técnicos en B**; managers fuera de las métricas; promediar solo dimensiones evaluables y decir "n de N". Piezas: `queries.sql` (Q0–Q9, tablas directo — los RPC de equipo devuelven `UNAUTHORIZED` desde `execute_sql`), `build-report-b.mjs` + `report-b-skeleton.html` (JSON de datos → HTML), `report-a-template.md`. Datos por piloto en `docs/piloto/<empresa>-<fecha>.data.json`. Definicion: `.claude/skills/piloto-analisis/SKILL.md`
...
> - **`test_checkpoint_creation` está `#[ignore]`**: hace un encode y un concat reales, y en una máquina sin ffmpeg `find_ffmpeg_path()` intentaría **descargarlo (287 MB)**; peor, tokio bloquea el `Drop` del runtime del test hasta que termine el `spawn_blocking`, así que se colgaría en vez de fallar rápido. Los dos tests nuevos prueban el cambio sin tocar ffmpeg: uno sostiene el permiso a mano y verifica que no se pierde audio, el otro usa la costura `take_buffer_for_checkpoint()` para comprobar la preasignación.
> - Pendiente anotado: `-movflags +faststart` reescribe cada intermedio, y `merge_checkpoints`/`recover_audio_from_checkpoints` corren un `std::process::Command` **bloqueante** dentro de una `async fn`.
...
> - **Condición de borrado = fila de `sync_queue` con `job_type='finalize_conversation' AND status='completed'` y `completed_at` de al menos N días.** Eso significa sincronizada **y** analizada (la cadena `depends_on` garantiza que los segmentos ya subieron). Esas filas no se purgan nunca: `cleanup_old_completed` no tiene un solo call site en producción. La verdad de la ruta en disco es `meetings.folder_path`, **no** `preferences.save_folder`: `initialize_meeting_folder` usa `get_default_recordings_folder()` e ignora la preferencia del usuario.
