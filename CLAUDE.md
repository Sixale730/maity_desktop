...
- **Salir, cerrar sesión, instalación rival o apagar YA NO suprimen la jornada hasta medianoche (0.2.62, #83)**: `close_owned_segment_for_exit` retiene el fin de sesión (`SessionEnd`) antes del stop, así que un logout+login en el mismo proceso rearranca la jornada en ≤1 tick. El paro manual del usuario y "día cerrado" SÍ sobreviven al reinicio, pero solo esos dos — ninguna ruta de salida/logout/apagado escribe supresión nueva a disco.
...
- **Tema claro por defecto (sep-2026, paridad web)**: `public/theme-boot.js` (script externo por la CSP) + `DashboardTheme` como ÚNICO escritor de `.dark`/`data-portal-theme`; `useTheme() → {theme, toggle}`; sin paletas `theme-*`. Fondos de `html` SOLO en `html[data-portal-theme=…]` (en `html.dark` rompen la transparencia de las ventanas aux). Las aux (coach-float, widget, device-picker) SIGUEN el tema vía `public/aux-theme-boot.js` + `lib/auxTheme.ts::useAuxThemeSync` (solo `.dark`, nunca `data-portal-theme`) y siempre `bg-transparent`; el ÚNICO toggle de tema es el de la Sidebar (se quitó del chat y del dashboard). Colores nuevos = tokens; guard de una dirección `src/test/themeTokens.test.ts`.
...
**Ciclo de vida del proceso y de la jornada (0.2.62, issue #83 "¿por qué no grabó?"):** `app.start`/`app.exit`/`app.resumed` salen de un **marcador síncrono en disco** (`lifecycle.json` en `app_local_data_dir`, NUNCA `telemetry.json` del plugin-store — no es atómico) que es la fuente de verdad; la fila del outbox es best-effort (`logging/telemetry/lifecycle.rs`). `app.exit` se escribe ANTES de todo lo demás y el insert al outbox está acotado a 750 ms — en fin de sesión de Windows (`WM_ENDSESSION`) el hilo principal queda acotado a <5 s, sin merge/finalize/espera de transcripción ni emits/getters de ventana/tray/notificaciones ahí. `health.heartbeat` (ambos emisores) y `device.profile` llevan `idle_reason` (dominio cerrado) y el bloque `jornada`; el scheduler emite `jornada.idle_reason_changed` por el outbox en cada transición y es la línea de tiempo preferente (sobrevive sin red). `jornada.settings_changed` sale del único punto de persistencia (`ScheduledRecordingService::update_settings`) y solo si hubo diff real. Autostart real en canal directo con `autostart.changed` (`autostart_state.rs::reconcile`, compara contra la línea base del marcador) y `disabledByUser` cuando Task Manager lo apagó. Todo logout pasa por `AuthContext.signOut` → `logout_cleanup` (nunca `supabase.auth.signOut()` directo — hay fitness test); `auth.logout` se emite desde Rust con `surface` y flush dirigido con el token de quien sale; `auth.session_lost` SOLO por `SIGNED_OUT` espontáneo o arranque sin sesión con marca previa y error no reintentable (nunca por red caída), y antes de soltar al usuario se detiene y guarda la grabación activa. Recargar el webview con sesión viva NO suelta al usuario en Rust. Detalle completo, payloads y query de clasificación: `docs/TELEMETRIA.md`.
