...
import type { MeetingRecord, ProcessTranscriptResult, SummaryPollingResult, LegacySummaryBlock, LegacySummarySection, OllamaModelEntry } from './types';
...
export function useSummaryGeneration({
  ...
  const processSummary = useCallback(async ({
    ...
    try {
      ...
      const result = await invokeTauri<ProcessTranscriptResult>('api_process_transcript', {
        text: transcriptText,
      ...
      startSummaryPolling(meeting.id, process_id, async (rawPollingResult) => {
        const pollingResult = rawPollingResult as SummaryPollingResult;
        ...
        if (pollingResult.status === 'cancelled') {
          ...
          try {
            const existingSummary = await invokeTauri<{ data?: Summary }>('api_get_summary', {
              meetingId: meeting.id
            });
        ...
        if (pollingResult.status === 'error' || pollingResult.status === 'failed') {
          ...
          if (isRegeneration) {
            try {
              ...
              if (existingSummary?.data) {
                logger.debug('Restored previous summary after regeneration failure');
                setAiSummary(existingSummary.data);
                setSummaryStatus('completed');
                setSummaryError(null);
          ...
          toast.error(`Error al ${isRegeneration ? 'regenerar' : 'generar'} resumen`, {
            description: errorMessage.includes('Connection refused')
              ? 'No se pudo conectar al servicio LLM. Asegúrate de que Ollama o tu proveedor LLM configurado esté ejecutándose.'
              : errorMessage,
          });

          // Auto-open model settings modal if model is missing
          if (isModelRequiredError && onOpenModelSettings) {
            logger.debug('🔧 Model required error detected, opening model settings...');
            onOpenModelSettings();
          }
        ...
        if (pollingResult.status === 'completed' && pollingResult.data) {
          ...
          if (pollingResult.data.markdown) {
            ...
            toast.success('¡Resumen generado exitosamente!', {
              description: 'Tu resumen de reunión está listo',
              duration: 4000,
            });
          ...
          const allEmpty = summarySections.every(([, section]) => {
            const s = section as LegacySummarySection;
              ...
              if (section && typeof section === 'object' && 'title' in section && 'blocks' in section) {
                ...
                if (Array.isArray(typedSection.blocks)) {
                  formattedSummary[key] = {
                    ...
                    blocks: typedSection.blocks.map((block: LegacySummaryBlock) => ({
                      ...block,
                      color: 'default',
                      content: block?.content?.trim() || ''
                    // eslint-disable-next-line @typescript-eslint/no-explicit-any -- legacy blocks may lack id/type fields
                    })) as any[]
                  };
                } else {
                  formattedSummary[key] = {
  ...
  const handleGenerateSummary = useCallback(async (customPrompt: string = '') => {
    ...
    if (isModelConfigLoading) {
      ...
      toast.info('Cargando configuración del modelo, por favor espera...');
      return;
    }
    ...
    if (modelConfig.provider === 'ollama') {
      try {
        ...
        const models = await invokeTauri<OllamaModelEntry[]>('get_ollama_models', { endpoint });
        ...
        if (isOllamaNotInstalledError(errorMessage)) {
          ...
          toast.error(
            'Error al verificar modelos Ollama. Asegúrate de que Ollama esté ejecutándose y descarga un modelo desde Configuración.',
            { duration: 5000 }
          );
        }
        return;
      ...
      try {
        ...
        if (!selectedModel) {
          toast.error('No hay modelo de IA integrada seleccionado', {
            description: 'Por favor selecciona un modelo en configuración',
            duration: 5000,
          });
        ...
        if (!isReady) {
          ...
          if (modelInfo) {
            ...
            if (status.type === 'downloading') {
              toast.info('Descarga de modelo en progreso', {
                description: `${selectedModel} se está descargando (${status.progress}%). Por favor espera hasta que se complete.`,
                duration: 5000,
              });
              return;
            }
            ...
            if (status.type === 'not_downloaded') {
              toast.error('Modelo de IA integrada no descargado', {
                description: `${selectedModel} necesita ser descargado. Por favor descárgalo en configuración de modelo.`,
                duration: 7000,
              });
              if (onOpenModelSettings) {
                onOpenModelSettings();
            ...
            if (status.type === 'corrupted' || status.type === 'error') {
              ...
              toast.error('Modelo de IA integrada no disponible', {
                description: `${errorDesc}. Por favor revisa la configuración del modelo.`,
                duration: 7000,
              });
              if (onOpenModelSettings) {
                onOpenModelSettings();
          ...
          toast.error('Modelo de IA integrada no listo', {
            description: 'Por favor asegúrate de que el modelo esté descargado en configuración',
            duration: 5000,
          });
          if (onOpenModelSettings) {
            onOpenModelSettings();
  ...
  const handleStopGeneration = useCallback(async () => {
    ...
    toast.info('Generación de resumen detenida', {
