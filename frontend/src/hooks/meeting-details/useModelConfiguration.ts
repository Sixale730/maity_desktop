...
export function useModelConfiguration({ serverAddress }: UseModelConfigurationProps) {
  ...
  useEffect(() => {
    const fetchModelConfig = async () => {
      ...
      try {
        ...
        // eslint-disable-next-line @typescript-eslint/no-explicit-any -- data gets extended with custom OpenAI fields
        const data = await invokeTauri<ModelConfig & Record<string, any>>('api_get_model_config', {});
        if (data && data.provider !== null) {
          ...
          if (data.provider === 'custom-openai') {
            try {
              const customConfig = await invokeTauri<{
                displayName?: string;
                endpoint?: string;
                model?: string;
                apiKey?: string;
  ...
  const handleSaveModelConfig = useCallback(async (updatedConfig?: ModelConfig) => {
    try {
      ...
      toast.error("Error al guardar configuración de resumen", { description: String(error) });
      if (error instanceof Error) {
        setError(error.message);
      } else {
        setError('Error al guardar configuración del modelo: Error desconocido');
      }
    }
  }, [modelConfig]);
