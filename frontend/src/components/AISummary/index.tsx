...
export const AISummary = ({ summary, status, error, onSummaryChange, onRegenerateSummary, meeting }: Props) => {
  ...
  const currentSummary = useMemo(() => {
    if (!summary) {
      return {
        ...
        Decisions: { title: "Decisiones", blocks: [] },
        ActionItems: { title: "Acciones Pendientes", blocks: [] },
        ClosingRemarks: { title: "Conclusiones", blocks: [] }
      };
    }
  ...
  const convertToMarkdown = () => {
    let markdown = `# Resumen de Reunión Generado por IA: ${meeting?.id || 'Desconocido'} - ${meeting?.title || 'Reunión Sin Título'}\n\n`;
    markdown += `## Fecha: ${meeting?.created_at ? new Date(meeting.created_at).toLocaleDateString() : new Date().toLocaleDateString()}\n\n`;
    
    Object.entries(currentSummary).forEach(([key, section]) => {
      if (key === 'title') {
        markdown = `# ${section.title || 'Resumen Mejorado por IA'}\n\n`;
      } else {
  ...
  const renderErrorState = () => (
    <div className="w-full p-4 bg-[#fff0f5] dark:bg-red-900/30 border border-[#ffc0d6] dark:border-red-700 rounded-lg">
      <div className="flex items-center mb-2">
        <ExclamationTriangleIcon className="h-5 w-5 text-[#ff0050] dark:text-red-400 mr-2" />
        <h3 className="text-[#990030] dark:text-red-300 font-medium">Error Generando Resumen</h3>
      </div>
      <p className="text-[#cc0040] dark:text-red-400 text-sm">{error}</p>
      <p className="text-[#ff0050] dark:text-red-500 text-xs mt-2">Por favor verifica la configuración del modelo y las claves API, o inténtalo de nuevo.</p>
    </div>
  );

  const renderLoadingState = () => (
    <div className="w-full p-4 bg-[#f0f2fe] dark:bg-blue-900/30 border border-[#c0cbfb] dark:border-blue-700 rounded-lg">
      <div className="flex items-center space-x-3">
        <div className="animate-spin rounded-full h-5 w-5 border-2 border-[#485df4] border-t-transparent"></div>
        <div>
          <h3 className="text-[#2b3892] dark:text-blue-300 font-medium">
            {status === 'processing' ? 'Procesando Transcripción' : 'Generando Resumen'}
          </h3>
          <p className="text-[#3a4ac3] dark:text-blue-400 text-sm">
            {status === 'processing'
              ? 'Analizando tu transcripción...'
              : 'Creando un resumen detallado de tu reunión...'}
          </p>
        </div>
      </div>
    </div>
      ...
      {/* <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center space-x-2">
          <button
            ...
            className="p-2 hover:bg-[#e7e7e9] rounded disabled:opacity-50"
            title="Undo"
          >
            <svg
          ...
          <button
            ...
            className="p-2 hover:bg-[#e7e7e9] rounded"
          ...
          <button
            ...
            className="px-2 py-1 text-sm bg-[#e7e7e9] hover:bg-[#d0d0d3] rounded-md flex items-center space-x-1"
          >
            <span>📋</span>
            <span>Copiar</span>
          </button>
