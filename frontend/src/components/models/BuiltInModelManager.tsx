...
export function BuiltInModelManager({ selectedModel, onModelSelect }: BuiltInModelManagerProps) {
    ...
    const setupListener = async () => {
      ...
      unlisten = await listen('builtin-ai-download-progress', (event: any) => {
        ...
        if (status === 'completed') {
          ...
          // Refresh models list
          fetchModels();
          toast.success(`Modelo ${model} descargado exitosamente`);
        }
  ...
  const downloadModel = async (modelName: string) => {
    try {
      ...
      toast.error(`Error al descargar ${modelName}`);
  ...
  const cancelDownload = async (modelName: string) => {
    try {
      ...
      toast.info(`Descarga de ${modelName} cancelada`);
      setDownloadingModels((prev) => {
        const newSet = new Set(prev);
        newSet.delete(modelName);
        return newSet;
  ...
  const deleteModel = async (modelName: string) => {
    try {
      ...
      toast.error(`Error al eliminar ${modelName}`);
    }
  };
  ...
  if (hasFetched && models.length === 0) {
    return (
      <Alert>
        <AlertDescription>
          No se encontraron modelos. Descarga un modelo para comenzar con IA integrada.
        </AlertDescription>
      </Alert>
    );
  }
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        <h4 className="text-sm font-bold">Modelos de IA Integrada</h4>
      </div>
