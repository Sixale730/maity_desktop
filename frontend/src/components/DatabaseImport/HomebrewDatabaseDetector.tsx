...
export function HomebrewDatabaseDetector({ onImportSuccess: _onImportSuccess, onDecline }: HomebrewDatabaseDetectorProps) {
  ...
  return (
    <div className="mb-4 p-4 bg-maity-blue/10 border-2 border-maity-blue/40 rounded-lg">
      <div className="flex items-start gap-3">
        <Database className="h-6 w-6 text-maity-blue mt-0.5 flex-shrink-0" />
        <div className="flex-1">
          <div className="flex items-center gap-2 mb-1">
            <AlertCircle className="h-4 w-4 text-maity-blue" />
            <h3 className="text-sm font-semibold text-[#141d4a] dark:text-[#c0cbfb]">
              ¡Instalación anterior de Maity detectada!
            </h3>
          </div>
          <p className="text-sm text-[#1e2a6e] dark:text-[#c0cbfb] mb-2">
          ...
          <div className="bg-card/50 rounded p-2 mb-3">
            <p className="text-xs text-[#2b3892] dark:text-[#a0b0f9] font-mono break-all">
              {detectedPath}
            </p>
            <p className="text-xs text-maity-blue mt-1">
              Tamaño: {formatFileSize(dbSize)}
            </p>
          </div>
          <p className="text-sm text-[#1e2a6e] dark:text-[#c0cbfb] mb-3">
          ...
          <div className="flex gap-2">
            ...
            <button
              onClick={handleNo}
              disabled={isImporting}
              className="flex-1 px-4 py-2 border-2 border-maity-blue/50 text-[#2b3892] dark:text-[#a0b0f9] rounded-lg hover:bg-maity-blue/15 disabled:bg-card-hi disabled:cursor-not-allowed transition-colors"
            >
