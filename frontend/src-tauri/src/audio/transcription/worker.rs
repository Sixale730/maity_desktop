...
pub fn start_transcription_task<R: Runtime>(
    ...
    tokio::spawn(async move {
        ...
        ///
        /// A.3 — When the queue is full and chunks have to be dropped, emits
        /// `transcription-backpressure` to the frontend so the user sees a
        /// visible indicator. Previously drops were only written to logs,
        /// which is invisible during a recording session.
        async fn dispatch_accumulated<R: Runtime>(
            ...
            // Helper to emit a `transcription-backpressure` event with a
            // consistent shape. The field `recording_seconds_affected` lets
            // the UI tell the user "~N seconds of audio were not transcribed"
            // in plain language.
            ...
            match work_sender.try_send(accumulated) {
                ...
                Err(tokio::sync::mpsc::error::TrySendError::Full(chunk)) => {
                    ...
                    if drained > 0 {
                        ...
                        let queue_depth =
                            chunks_queued.load(Ordering::Relaxed).saturating_sub(dropped_total);
                        emit_backpressure(
                            app,
                            drained,
                            dropped_total,
                    ...
                    match work_sender.try_send(chunk) {
                        ...
                        Err(tokio::sync::mpsc::error::TrySendError::Full(c)) => {
                            ...
                            chunks_dropped.fetch_add(1, Ordering::SeqCst);
                            let dropped_total = chunks_dropped.load(Ordering::Relaxed);
                            let queue_depth = chunks_queued
                                .load(Ordering::Relaxed)
                                .saturating_sub(dropped_total);
                            emit_backpressure(
                                app,
                                1,
                                dropped_total,
        ...
        loop {
            ...
            // A.3 — Lag warning watermark: queue between 1000 and 1500 is the
            // "yellow zone" where drops are imminent but have not happened yet.
            // Emitting once every 10s gives the UI a chance to warn the user
            // *before* chunks get dropped. Reuses `last_lag_emit` implicitly via
            // the 3s interval above — an additional gating variable would be
            // overkill since the main emitter runs frequently enough.
            const LAG_WARNING_LOW: u64 = 1000;
            if pending >= LAG_WARNING_LOW && pending < backpressure_threshold {
                // Gate emissions at ~every 100 pending chunks to avoid spam.
                if pending % 100 == 0 {
                    let _ = app.emit(
