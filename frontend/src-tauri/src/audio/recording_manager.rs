...
impl RecordingManager {
    ...
    ///
    /// RACE CONDITION FIX: This function now checks if a reconnection is already
    /// in progress to prevent multiple concurrent reconnection attempts that could
    /// cause stream corruption or panic.
    pub async fn attempt_device_reconnect(&mut self, device_name: &str, device_type: DeviceMonitorType) -> Result<bool> {
        // Guard against concurrent reconnection attempts
        // Only proceed if we're already in reconnecting state (set by handle_device_disconnect)
        if !self.state.is_reconnecting() {
            info!("🔄 Reconnection skipped: not in reconnecting state");
            return Ok(false);
        }
    ...
    pub async fn handle_device_disconnect(&mut self, device_name: String, device_type: DeviceMonitorType) {
        ...
        if let Some(device) = device {
            ...
            // start_reconnecting now returns bool to prevent race conditions
            if !self.state.start_reconnecting(device, recording_device_type) {
                warn!("📱 Reconnection already in progress, skipping duplicate disconnect event");
            }
        }
    }
