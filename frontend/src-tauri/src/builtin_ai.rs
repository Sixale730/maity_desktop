...
/// Open the built-in AI models folder in the system file explorer.
/// Renombrado de `open_models_folder` para no chocar con el de whisper_engine.
#[tauri::command]
pub async fn open_builtin_models_folder<R: Runtime>(app: AppHandle<R>) -> Result<(), String> {
    let app_data_dir = app
        .path()
        .app_data_dir()
        .map_err(|e| format!("Failed to get app data dir: {}", e))?;
