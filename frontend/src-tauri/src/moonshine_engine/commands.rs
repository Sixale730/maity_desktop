...
#[command]
pub async fn moonshine_init() -> Result<(), String> {
    let mut guard = MOONSHINE_ENGINE.lock().unwrap();
    if guard.is_some() {
        return Ok(());
    }

    let models_dir = get_models_directory();
    let engine = MoonshineEngine::new_with_models_dir(models_dir)
        .map_err(|e| format!("Failed to initialize Moonshine engine: {}", e))?;
    *guard = Some(Arc::new(engine));
    Ok(())
}

#[command]
pub async fn moonshine_get_available_models() -> Result<Vec<ModelInfo>, String> {
    let engine = {
    ...
    if let Some(engine) = engine {
        engine
            .discover_models()
            .await
            .map_err(|e| format!("Failed to discover Moonshine models: {}", e))
    } else {
        Err("Moonshine engine not initialized".to_string())
    }
}
    ...
    if let Some(engine) = engine {
        ...
        if result.is_ok() {
            ...
            if let Err(e) = app_handle.emit(
                ...
                log::error!("Failed to emit moonshine-model-loading-failed event: {}", e);
            }
        }
...
#[command]
pub async fn moonshine_get_current_model() -> Result<Option<String>, String> {
    let engine = {
        let guard = MOONSHINE_ENGINE.lock().unwrap();
        guard.as_ref().cloned()
    };
...
#[command]
pub async fn moonshine_is_model_loaded() -> Result<bool, String> {
    let engine = {
        let guard = MOONSHINE_ENGINE.lock().unwrap();
        guard.as_ref().cloned()
    };
...
#[command]
pub async fn moonshine_has_available_models() -> Result<bool, String> {
    let engine = {
        let guard = MOONSHINE_ENGINE.lock().unwrap();
        guard.as_ref().cloned()
    };
...
#[command]
pub async fn moonshine_validate_model_ready() -> Result<String, String> {
    let engine = {
        let guard = MOONSHINE_ENGINE.lock().unwrap();
        guard.as_ref().cloned()
    };
    ...
    if let Some(engine) = engine {
        ...
        if available_models.is_empty() {
            return Err(
                "No Moonshine models are available. Please download a model to enable transcription."
                    .to_string(),
            );
        }
        ...
        engine
            ...
            .await
            .map_err(|e| format!("Failed to load Moonshine model {}: {}", first_model.name, e))?;
...
/// Internal version of moonshine_validate_model_ready that respects user's transcript config
pub async fn moonshine_validate_model_ready_with_config<R: tauri::Runtime>(
    ...
    if let Some(engine) = engine {
        ...
        if engine.is_model_loaded().await {
            if let Some(current_model) = engine.get_current_model().await {
                log::info!("Moonshine model already loaded: {}", current_model);
                return Ok(current_model);
            }
        }
        ...
        .await
        {
            Ok(Some(config)) => {
                ...
                if config.provider == "moonshine" && !config.model.is_empty() {
                    log::info!("Using user's configured Moonshine model: {}", config.model);
                    Some(config.model)
                } else {
                    log::info!(
                        "API config uses non-Moonshine provider ({}) or empty model, will auto-select",
                        config.provider
                    );
                    None
                }
            ...
            Ok(None) => {
                log::info!("No transcript config found in API, will auto-select Moonshine model");
                None
            }
            Err(e) => {
                log::warn!(
                    "Failed to get transcript config from API: {}, will auto-select Moonshine model",
                    e
                );
                None
            }
        ...
        let model_name = if let Some(configured_model) = model_to_load {
            ...
            if available_models.iter().any(|m| m.name == configured_model) {
                ...
                log::warn!(
                    "Configured Moonshine model '{}' not found, falling back to first available model",
                    configured_model
                );
                available_models.first().unwrap().name.clone()
            }
        } else {
            // No configured model, load first available
            log::info!("No configured model, loading first available Moonshine model");
            available_models.first().unwrap().name.clone()
        };
        ...
        engine
            .load_model(&model_name)
            .await
            .map_err(|e| format!("Failed to load Moonshine model {}: {}", model_name, e))?;
...
#[command]
pub async fn moonshine_transcribe_audio(audio_data: Vec<f32>) -> Result<String, String> {
    let engine = {
        let guard = MOONSHINE_ENGINE.lock().unwrap();
        guard.as_ref().cloned()
    };
...
#[command]
pub async fn moonshine_get_models_directory() -> Result<String, String> {
    let engine = {
        let guard = MOONSHINE_ENGINE.lock().unwrap();
        guard.as_ref().cloned()
    };
...
#[command]
pub async fn moonshine_download_model<R: Runtime>(
    app_handle: AppHandle<R>,
    model_name: String,
) -> Result<(), String> {
    let engine = {
    ...
    if let Some(engine) = engine {
        ...
        let progress_callback = Box::new(move |progress: DownloadProgress| {
            ...
            if let Err(e) = app_handle_clone.emit(
                ...
                log::error!("Failed to emit moonshine download progress event: {}", e);
            }
        });
        ...
        match result {
            Ok(()) => {
                ...
                if let Err(e) = app_handle.emit(
                    ...
                    log::error!("Failed to emit moonshine download complete event: {}", e);
                }

                // Update tray menu to reflect model is now available
                log::info!("Moonshine model download complete - updating tray menu");
            ...
            Err(e) => {
                ...
                if let Err(emit_e) = app_handle.emit(
                    ...
                    log::error!("Failed to emit moonshine download error event: {}", emit_e);
                }
                Err(format!("Failed to download Moonshine model: {}", e))
            }
        }
    } else {
        Err("Moonshine engine not initialized".to_string())
...
#[command]
pub async fn moonshine_cancel_download<R: Runtime>(
    app_handle: AppHandle<R>,
    model_name: String,
) -> Result<(), String> {
    let engine = {
    ...
    if let Some(engine) = engine {
        engine
            ...
            .await
            .map_err(|e| format!("Failed to cancel Moonshine download: {}", e))?;
        ...
        log::info!("Moonshine download cancelled: {}", model_name);
        Ok(())
    } else {
        Err("Moonshine engine not initialized".to_string())
    }
...
#[command]
pub async fn moonshine_retry_download<R: Runtime>(
    app_handle: AppHandle<R>,
    model_name: String,
) -> Result<(), String> {
    ...
    if let Some(engine) = engine {
        ...
        moonshine_download_model(app_handle, model_name).await
    } else {
        Err("Moonshine engine not initialized".to_string())
    }
}
...
#[command]
pub async fn moonshine_delete_corrupted_model(model_name: String) -> Result<String, String> {
    let engine = {
        let guard = MOONSHINE_ENGINE.lock().unwrap();
        guard.as_ref().cloned()
    };
    ...
    if let Some(engine) = engine {
        engine
            ...
            .await
            .map_err(|e| format!("Failed to delete Moonshine model: {}", e))
    } else {
        Err("Moonshine engine not initialized".to_string())
    }
}

/// Open the Moonshine models folder in the system file explorer
#[command]
pub async fn open_moonshine_models_folder() -> Result<(), String> {
    let models_dir = get_models_directory()
        .ok_or_else(|| "Moonshine models directory not initialized".to_string())?
        .join("moonshine");
    ...
    log::info!("Opened Moonshine models folder: {}", folder_path);
    Ok(())
}
