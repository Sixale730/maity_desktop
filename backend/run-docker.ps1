...
$AppProjectName = "maity-backend"
$AppContainerName = "maity-backend"
$DefaultPort = 8178
$DefaultAppPort = 5167
...
function Invoke-StartCommand {
    ...
    $composeArgs += @("whisper-server", "maity-backend")
    ...
    if ($DryRun) {
        ...
        if ($detach) {
            ...
            & docker-compose -f $ComposeFile --profile default up -d whisper-server maity-backend
            ...
            & docker-compose -f $ComposeFile --profile default up whisper-server maity-backend
...
function Invoke-LogsCommand {
    ...
    $serviceMap = @{
        ...
        "app" = "maity-backend"
        "backend" = "maity-backend"
        "meeting" = "maity-backend"
    }
    ...
    if ($service) {
        ...
        $validServices = @("whisper-server", "maity-backend")
        if ($service -notin $validServices) {
            Write-Warn "Unknown service: $service"
            Write-Info "Available services: whisper-server, maity-backend"
        ...
        $logArgs += @("whisper-server", "maity-backend")
    }
