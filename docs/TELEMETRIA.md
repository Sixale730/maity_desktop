...
| `stt.batch_job` | Un job de transcripción por lote terminó (`batch/planner.rs::process_queue`; una fila por job terminal, éxito o fallo) | `trigger` = `manual` \| `rotation` \| `auto_close` \| `crash_recovery`; `attempts`; con éxito además `audio_secs`, `voiced_secs`, `wall_ms`, `rtf`, `words`, `segments`; `status` = `ok` \| `error` |
