...
- **Migración ADITIVA** `20260909000000_add_batch_transcription_queue.sql`: `folder_path UNIQUE`, `meeting_local_id`, `segment_started_at`, `trigger_kind (manual|rotation|auto_close|crash_recovery)`, `status (recording|pending|processing|done|discarded|failed)`, `attempts`, `last_error`, `user_id`; + `process_id` (migración `20260911100000`, 2026-09-11: pid del proceso que graba, discriminador de huérfanos). Filas terminales se CONSERVAN (regla #26).
- **`database/repositories/batch_queue.rs`**: upsert idempotente por carpeta (re-sella `process_id`), claim-mutex, fail con reintentos (5, solo `processing`) / fail_recovery (merge sobre `recording`) / fail_permanent, reset_processing, crash_recovery (`attempts=0`). Tests in-memory.
- **`batch/planner.rs`**: tarea propia (delay 120 s); `notify_enqueued()` + tick 5 min + `request_drain()`; **gate híbrido puro**: `pressure==Normal && avail > headroom (1800 frío / 400 warm) && CPU <80%`; streaming activo SIEMPRE difiere; drain solo se detiene con `Critical`; diferir no quema attempts; single-flight; al vaciar → `unload_stt("batch_done")`. Recuperación: `processing`→`pending` al arranque; huérfanos (`is_orphan`: `process_id` ajeno, o propio con fase `Idle`) al arranque **y en cada despertar del loop** → checkpoints-first (`recovery_action`) → `crash_recovery` (`attempts=0`) | `failed`; error del merge → `fail_recovery` (5 reintentos en `recording`); backlog→drain. **Ya no se pospone por grabación activa** (2026-09-11; detalle en `TRANSCRIPTION_PIPELINE.md` § F2).
...
- **`contexts/TranscriptContext.tsx`**: el registro WAL de IndexedDB lleva `transcriptionMode` leído de `get_recording_state`; `TranscriptPanel` muestra `BatchRecordingHero` en lugar de la vista en vivo (sep-2026; antes sólo el texto "Transcribiendo al finalizar la grabación", que dejaba la pantalla vacía — ver `docs/UI_REGLAS.md` § "Pantalla de grabación en lote").
...
- **`coach/audio_heuristics.rs`** (puro): `AudioSnapshot::from_voice` (`is_monologue`, `health_score`, `user_talk_ratio`), `AudioTipState`, `evaluate_audio_tips` → dos tips: monólogo (`>60 s` `audio_monologue_long`; `>150 s` crítico `audio_monologue_critical`) y dominancia (`>70 %` con interlocutor presente, sesión ≥120 s, voz total ≥60 s, cada 5 min). **Actualización 2026-09-11:** en modo ponente ninguno de los dos aplica; la tabla de ponente (rachas 5/10 min, audiencia callada 10 min sólo si ya intervino, Ritmo 50-75) vive en `coach/presenter_heuristics.rs` para audio y transcript, y el tracker publica además `last_interlocutor_voice_ms` (detalle en `docs/COACH_LLM_ARCHITECTURE.md`).
