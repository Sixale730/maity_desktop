...
La transcripcion en la nube usa Deepgram a traves de un Cloudflare Worker proxy. **La API key de Deepgram nunca llega al cliente**: el token lo emite Vercel `GET /api/conversations?action=deepgram-token` (JWT de 5 min) y el Worker `maity-deepgram-proxy` es el único que conoce la key.

**Ruta del token (sep-2026, #82)**: antes era `/api/deepgram-token`; la web la retiró para liberar un slot de función de Vercel Hobby y NO mantiene alias. Por el rewrite catch-all de `vercel.json` de la web, la ruta vieja no da 404 sino **200 con el HTML de la SPA** — en un build anterior al fix el fallback de Deepgram falla con `server:Respuesta del servidor inválida`, no con un error claro. La respuesta de la ruta nueva es idéntica (`{ mode: 'proxy', ws_url, config }`, JWT en el query `token` de `ws_url`; 401/429/5xx iguales).
